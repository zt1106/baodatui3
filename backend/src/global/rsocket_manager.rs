...
impl RSocketManager {
        ...
        Res: Serialize + DeserializeOwned + 'static,
    {
        ...
        self.raw_req_handler_map.write().insert(command, Arc::new(handler_wrapper));
    }

    pub fn raw_handler(&self, command: &str) -> Arc<dyn RawRequestHandler + Send + Sync> {
        self.raw_req_handler_map.read().get(command).unwrap().clone()
    }

    pub fn add_stream_handler<Req, T>(&self, command: impl Into<String>, handler: impl StreamHandler<Req, T> + 'static)
    where
        ...
        T: Serialize + DeserializeOwned + Send + 'static,
    {
        ...
        self.raw_stream_handler_map.write().insert(command, Arc::new(wrapper));
    }

    pub fn raw_stream_handler(&self, command: &str) -> Arc<dyn RawStreamHandler + Send + Sync> {
        self.raw_stream_handler_map.read().get(command).unwrap().clone()
    }
}